...
└── sign_language/            # Sign Language CNN module
    ├── model.py              # CNN architecture (3x Conv2D, 64x64 RGB, 10 classes)
    ├── train_sign_digits.py  # Training script
    ├── evaluate_sign_digits.py
    ├── visualize_sign_digits.py  # 6x6 prediction grid
...
| File | Role |
|------|------|
| `chatbot/chatbot.py` | `ChatbotModel` (nn.Module) + `ChatbotAssistant` class |
| `chatbot/database.py` | MySQL layer: auth, history, dynamic intents, formatting |
| `chatbot/main.py` | Tkinter app: login window + chatbot UI |
| `chatbot/chatbot.sql` | Full SQL schema |
...
Trains a CNN on the classic [MNIST](http://yann.lecun.com/exdb/mnist/) dataset (60k handwritten digits, 28x28 grayscale).
...
Trains a CNN to classify **sign language hand gesture images** (digits 0–9) from the
...
- `visualize_sign_digits.py` renders a 6x6 prediction grid (green = correct, red = wrong)
...
Log in or register then ask about events, services or transport lines.
